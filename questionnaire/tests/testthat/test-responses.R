...
test_that("answers are appended to the CSV and read back", {
  ...
  append_row(new_response("2026-09-01", "Ana", "Linearity", "Regular", "Analyzer 01", "P1", "L1"),
    path
  )
  second <- new_response(
    as.Date("2026-09-02"), "Ben", "Linearity", "Retest", "Analyzer 02", "P2", "L2"
  )
...
test_that("a file from an older version (no instrument column) is upgraded", {
  ...
  append_row(new_response("2026-09-01", "Ana", "Linearity", "Pre-test", "Analyzer 01", "P2", "L2"),
    path
  )
